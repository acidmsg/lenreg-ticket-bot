...
if [ "${RESTORE_IN_CONTAINER}" = "false" ]; then
    ...
    if docker compose -f /root/zdrav.lenreg/docker-compose.yml stop bot 2>/dev/null; then
...
if [ "${RESTORE_IN_CONTAINER}" = "false" ]; then
    ...
    if docker compose -f /root/zdrav.lenreg/docker-compose.yml start bot 2>/dev/null; then
        echo "[RESTORE] Бот запущен"
    else
        echo "[WARN] Не удалось запустить бота через docker compose"
        echo "[WARN] Запустите вручную: docker compose -f /root/zdrav.lenreg/docker-compose.yml start bot"
    fi
else
