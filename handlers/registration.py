...
async def process_fio(message: Message, state: FSMContext):
    ...
    if len(parts) != 3:
        await message.answer("Ошибка! ФИО должно состоять строго из 3 слов (Фамилия Имя Отчество).")
        return
...
async def process_bday(message: Message, state: FSMContext, api: ZdravClient):
    ...
    try:
        ...
        await message.answer("Неверная дата. Введите корректную дату в формате дд.мм.гггг (с 01.01.1900 по сегодня).")
        return

    data = await state.get_data()
    fio = data['fio']

    # Для проверки пациента берем тестовую клинику 272 (или любую другую)
    clinic_id = "272"
