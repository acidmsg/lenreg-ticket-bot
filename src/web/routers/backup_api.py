...
Все эндпоинты защищены сессией дашборда, как и остальной API
(``SessionAuthMiddleware``), а небезопасные методы дополнительно требуют
CSRF-токен. Путь ``/api/backups/*`` не входит в публичные префиксы
(``/app``, ``/static``, ``/api/user``).
