/**
 * Утилита иконок Lucide — генерирует инлайн SVG по имени.
 * Все SVG-строки хранятся в объекте SVG_CONTENT (только inner-элементы).
 * Кэширование не требуется (строки уже жёстко закодированы).
 *
 * @module components/icon
 */

/** SVG-содержимое иконок Lucide (внутренние элементы, без обёртки <svg>). */
const SVG_CONTENT = {
  ...
  moon: '<path d="M12 3a6.36 6.36 0 0 0 9 9 9 9 0 1 1-9-9"/>',
  monitor:
    '<rect width="20" height="14" x="2" y="3" rx="2"/><line x1="8" x2="16" y1="21" y2="21"/><line x1="12" x2="12" y1="17" y2="21"/>',
};

/** Обёртка SVG для генерации полной иконки. */
const SVG_WRAPPER =
  '<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">{c}</svg>';

/**
 * Возвращает HTML-строку с инлайн SVG-иконкой Lucide.
 *
 * @param {string} name — имя иконки (ключ в SVG_CONTENT)
 * @param {number} [size=20] — размер иконки в px
 * @returns {string} HTML-строка SVG или '', если иконка не найдена
 */
export function lucideIcon(name, size = 20) {
  const content = SVG_CONTENT[name];
  if (!content) return "";
  return SVG_WRAPPER.replace("{w}", String(size))
