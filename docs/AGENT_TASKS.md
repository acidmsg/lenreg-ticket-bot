...
### Скрытие кнопок управления при пустом списке пациентов (сессия 2026-05-10, ч.5)

| # | Задача | Статус | Примечание |
|---|---|---|---|
| 89 | `keyboards/inline.py` — кнопка "Сбросить весь мониторинг" только при наличии пациентов | ✅ | `if patients:` |
| 90 | `handlers/common.py` — `back_to_main()` показывает приветствие если пациентов нет | ✅ | Только кнопка "➕ Добавить пациента" |
| 91 | `handlers/common.py` — `handle_delete_patient()` показывает приветствие после удаления последнего | ✅ | |

### Единый формат сообщений о номерках (эмодзи + вёрстка)

| # | Задача | Статус | Примечание |
|---|---|---|---|
| 92 | Добавить эмодзи во все заголовки уведомлений о номерках | ✅ | `services/monitor.py` — 🎉, ⚠️, 🤷‍♂️, 🔗 |
| 93 | Унифицировать вёрстку (🧑‍⚕️ + пробел, без :, заглавная П, без "они") | ✅ | `handlers/common.py`, `services/monitor.py` |
| 94 | Обновлены логи, AGENT_TASKS, сделан коммит | ✅ | |
