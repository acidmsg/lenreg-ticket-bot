...
- **VPS:** 195.58.39.52, root, проект в `/srv/bots/lenreg-ticket-bot`
...
```bash
sqlite3 /srv/bots/lenreg-ticket-bot/data/bot.db ".backup '/root/backups/daily/bot_20260614.db'"
```
...
```text
0 3 * * * /srv/bots/lenreg-ticket-bot/scripts/backup.sh >> /root/backups/backup.log 2>&1
```
...
0 0 * * 0 /srv/bots/lenreg-ticket-bot/scripts/backup.sh weekly >> /root/backups/backup.log 2>&1

# Месячный бэкап (1-е число)
0 0 1 * * /srv/bots/lenreg-ticket-bot/scripts/backup.sh monthly >> /root/backups/backup.log 2>&1
```
...
docker compose -f /srv/bots/lenreg-ticket-bot/docker-compose.yml stop bot

# 2. Сделать резервную копию текущей (возможно, повреждённой) БД
cp /srv/bots/lenreg-ticket-bot/data/bot.db /srv/bots/lenreg-ticket-bot/data/bot.db.broken
cp /srv/bots/lenreg-ticket-bot/data/bot.db-wal /srv/bots/lenreg-ticket-bot/data/bot.db-wal.broken 2>/dev/null || true
cp /srv/bots/lenreg-ticket-bot/data/bot.db-shm /srv/bots/lenreg-ticket-bot/data/bot.db-shm.broken 2>/dev/null || true

# 3. Удалить WAL/SHM (будут созданы заново)
rm -f /srv/bots/lenreg-ticket-bot/data/bot.db-wal /srv/bots/lenreg-ticket-bot/data/bot.db-shm

# 4. Скопировать бэкап на место
cp /root/backups/daily/bot_20260613.db /srv/bots/lenreg-ticket-bot/data/bot.db

# 5. Проверить целостность восстановленной БД
sqlite3 /srv/bots/lenreg-ticket-bot/data/bot.db "PRAGMA integrity_check"
# Должно вывести: ok

# 6. Права доступа (важно: контейнер работает под appuser)
chown -R 1000:1000 /srv/bots/lenreg-ticket-bot/data/

# 7. Запустить бота
docker compose -f /srv/bots/lenreg-ticket-bot/docker-compose.yml start bot
```

### 10.2 Восстановление Redis (опционально)

```bash
docker compose -f /srv/bots/lenreg-ticket-bot/docker-compose.yml stop redis
cp /root/backups/redis/dump_20260613.rdb \
   /var/lib/docker/volumes/lenreg_ticket_redis_data/_data/dump.rdb
docker compose -f /srv/bots/lenreg-ticket-bot/docker-compose.yml start redis
```
...
sqlite3 /srv/bots/lenreg-ticket-bot/data/bot.db < /tmp/restore_user_patients.sql
```
...
| ---------------- | ---------------------------------- | ------------------------- |
| `PROJECT_DIR`    | `/srv/bots/lenreg-ticket-bot`      | `/app`                    |
| `BACKUP_DIR`     | `/root/backups`                    | `/app/backups`            |
| `DATA_DIR`       | `/srv/bots/lenreg-ticket-bot/data` | `/app/data`               |
