...
```text
lenreg-ticket-bot/                     # Корень проекта (только конфигурационные файлы)
...
Проект использует двухрепозиторную схему для разделения приватной инфраструктуры разработки и публичного релизного кода.

### Репозитории

| Роль               | Имя                       | Видимость | Содержимое                                                                                                                                                                              |
| ------------------ | ------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source of Truth    | `lenreg-ticket-bot--core` | Private   | 100% файлов: код, служебная инфраструктура, тесты, документация                                                                                                                         |
| Production Release | `lenreg-ticket-bot`       | Public    | Только чистый код (`src/`, `locales/`, `Dockerfile`, `docker-compose.yml`, `README.md`, `pyproject.toml`, `poetry.lock`, `Makefile`, `.env.example`, `.dockerignore`, `.gitattributes`) |

### Поток данных

1. Разработка ведётся исключительно в приватном репозитории `lenreg-ticket-bot--core`.
2. При публикации релиза (GitHub Release → Publish) срабатывает GitHub Actions workflow [`.github/workflows/release.yml`](.github/workflows/release.yml).
3. Workflow выполняет две задачи параллельно:
   - **sync_to_public:** копирует разрешённые файлы в публичный репозиторий `lenreg-ticket-bot` (ветка `main`).
   - **build_and_push_docker:** собирает Docker-образ и публикует в GitHub Container Registry (`ghcr.io`).

### Фильтрация файлов

Три уровня защиты от утечки служебных файлов:

| Уровень | Механизм                           | Файл                 | Назначение                                                                                                              |
| ------- | ---------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Git     | `.gitignore`                       | Корень проекта       | Исключает секреты (`.env`), логи, WAL-файлы SQLite из версионирования                                                   |
| Docker  | `.dockerignore`                    | Корень проекта       | Исключает `.roo/`, `.agents/`, `specs/`, `tests/`, `scripts/`, `_design_lab/`, `data/`, `logs/` из сборочного контекста |
| Release | `.gitattributes` (`export-ignore`) | Корень проекта       | Исключает служебные директории из `.zip`/`.tar.gz` архивов GitHub Releases                                              |
| CI/CD   | `release.yml` (фильтр копирования) | `.github/workflows/` | Копирует только разрешённые файлы в публичный репозиторий                                                               |
