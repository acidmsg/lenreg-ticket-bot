...
async def fetch_specialties(api: ZdravClient, patient_id: str, clinic_id: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        } for s in response if s.get("IdSpesiality") and s.get("NameSpesiality")]
    except Exception as e:
        logger.error(f"Ошибка получения специальностей для {clinic_id}: {e}", exc_info=True)
        return []

async def discovery_loop(api: ZdravClient, doctor_manager: DoctorManager, clinic_id: str, patient_id_adult: str, patient_id_child: str):
    ...
    while True:
        try:
            ...
            specialties_data = await fetch_specialties(api, current_patient_id, clinic_id)

            for specialty_info in specialties_data:
                spec_id = specialty_info["IdSpesiality"]
                spec_name = specialty_info["NameSpesiality"]

                doctors = await api.fetch_all_doctors(specialty_id=spec_id, patient_id=current_patient_id, clinic_id=str(clinic_id))
                if doctors:
                    for doc in doctors:
                        doc["SpesialityName"] = spec_name # Добавляем имя специальности к каждому врачу
                    all_doctors_with_specialty.extend(doctors)
                await asyncio.sleep(random.uniform(1.0, 3.0)) # Случайная пауза между запросами специальностей

            if all_doctors_with_specialty:
                await doctor_manager.merge_doctors(str(clinic_id), all_doctors_with_specialty)
                logger.info(f"Обновлен список врачей для {clinic_id}: всего {len(all_doctors_with_specialty)} записей")
